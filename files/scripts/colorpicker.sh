...
check_dependencies() {
  ! command -v hyprpicker &>/dev/null &&
    notify-senf -u critical -a "Color Picker" xcolor-pick "gpick needs to be installed" && exit 1
  
  !command -v magick &>/dev/null &&
    notify-senf -u critical -a "Color Picker" xcolor-pick "imagemagick needs to be installed" && exit 1
}
